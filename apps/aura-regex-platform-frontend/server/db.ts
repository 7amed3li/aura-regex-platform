...
import { InsertUser, users, folders, rules, testCases, generationLogs } from "../drizzle/schema";
...
export async function getTestCasesByRuleId(ruleId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(testCases).where(eq(testCases.ruleId, ruleId));
}

export async function getUserGenerationLogs(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(generationLogs).where(eq(generationLogs.userId, userId));
}
