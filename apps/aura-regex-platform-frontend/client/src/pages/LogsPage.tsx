// ⚠️ ملاحظة: هذا الملف LogsPage.tsx الذي يقوم بتصحيح خطأ 404
...
      setLogsLoading(true);
      
      // ✅ الحل المؤقت لخطأ 404: تغيير المسار إلى '/api/logs'
      // ملاحظة: المسار الصحيح يعتمد على ملف server.js/app.js لديك
...
      // عرض رسالة خطأ أكثر فائدة
      toast.error(`Error fetching logs: ${error.response?.status === 404 ? 'API route not found' : error.message}`); 
      setLogs([]); 
    } finally {
      setLogsLoading(false);
    }
...
  const handleCopyOutput = (output: string | null | undefined, logId: string) => {
    if (!output) {
